...
public class AdminFaqDraftController {
     ...
     * <p>이 경로는 FAQ 초안까지 자동 생성하는 것이 목적이므로, autoGenerateDrafts가 null이면 true로 설정합니다.</p>
     * 
    ...
    public AutoFaqGenerateResponse generateAuto(
        ...
        // /admin/faq/drafts/auto-generate 경로는 FAQ 초안까지 자동 생성하는 것이 목적이므로
        // autoGenerateDrafts가 null이면 true로 설정
        if (request.getAutoGenerateDrafts() == null) {
            request.setAutoGenerateDrafts(true);
            log.info("[FAQ 자동 생성] autoGenerateDrafts가 null이어서 true로 설정 (drafts 경로에서는 기본값 true)");
        }
